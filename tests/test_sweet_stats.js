#!/usr/bin/env node
// Tests for /sweet-stats — direct script invocation and via mode tracker.
// Run: node tests/test_sweet_stats.js
...
const STATS = path.join(ROOT, 'src', 'hooks', 'sweet-stats.js');
const TRACKER = path.join(ROOT, 'src', 'hooks', 'sweet-mode-tracker.js');

// sweet-stats.js ships with no measured compression ratios (see the comment on
// COMPRESSION there — the inherited English figure does not apply to Korean).
// These tests exercise the estimate arithmetic, not the constant's value, so
// they supply a ratio explicitly. Every child process below inherits it via
// { ...process.env }.
process.env.SWEET_COMPRESSION_RATIOS = '{"full":0.65}';

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sweet-stats-test-'));
  try {
    fn(tmp);
    passed++;
    console.log(`  ✓ ${name}`);
...
test('shows full-mode savings estimate when flag is full', (tmp) => {
  ...
  fs.writeFileSync(path.join(claudeDir, '.sweet-active'), 'full');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  // 350 / 0.35 = 1000, saved = 650, ~65%
  assert.match(out, /Est\. without sweet:\s+1,000/);
...
test('skips estimate for non-full modes', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.sweet-active'), 'ultra');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
...
test('mode tracker handles /sweet-stats with decision block', (tmp) => {
  const sess = makeSession(tmp, [
    { type: 'assistant', message: { usage: { output_tokens: 100 } } },
  ]);
  const claudeDir = path.join(tmp, '.claude');
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    ...
    input: JSON.stringify({ prompt: '/sweet-stats', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
  assert.match(parsed.reason, /Sweet Stats/);
  assert.match(parsed.reason, /Output tokens:\s+100/);
});

test('mode tracker preserves sweet flag when /sweet-stats fires', (tmp) => {
  const sess = makeSession(tmp, [
  ...
  assert.strictEqual(fs.readFileSync(path.join(claudeDir, '.sweet-active'), 'utf8'), 'full');
});
...
test('priceForModel matches by prefix across point releases', () => {
  const { priceForModel } = require(path.join(ROOT, 'src', 'hooks', 'sweet-stats.js'));
...
test('formatStats handles empty session gracefully', () => {
  const { formatStats } = require(path.join(ROOT, 'src', 'hooks', 'sweet-stats.js'));
...
test('--share prints single-line tweetable summary', (tmp) => {
  ...
  assert.match(out, /^🪨 Saved 650 output tokens \(~\$0\.009[78]\) across 1 turns this session — sweet\.sh$/m);
});
...
test('--share works with no benchmark ratio (lite mode)', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.sweet-active'), 'lite');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess, '--share'], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  assert.match(out, /^🪨 1 turns, 200 output tokens this session — sweet\.sh$/m);
});
...
test('appends to lifetime history on each run', (tmp) => {
  ...
  const histPath = path.join(claudeDir, '.sweet-history.jsonl');
...
test('skips pairs where compressed is not actually smaller', (tmp) => {
  const { findCompressedPairs } = require(path.join(ROOT, 'src', 'hooks', 'sweet-stats.js'));
...
test('writes statusline suffix file after a stats run', (tmp) => {
  ...
  const suffixPath = path.join(claudeDir, '.sweet-statusline-suffix');
...
test('humanizeTokens formats small/medium/large correctly', () => {
  const { humanizeTokens } = require(path.join(ROOT, 'src', 'hooks', 'sweet-stats.js'));
...
test('statusline.sh appends savings when SWEET_STATUSLINE_SAVINGS=1', (tmp) => {
  if (process.platform === 'win32') return; // bash test
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.sweet-active'), 'full');
  fs.writeFileSync(path.join(claudeDir, '.sweet-statusline-suffix'), '⛏ 2.8k');
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'sweet-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, SWEET_STATUSLINE_SAVINGS: '1' },
  });
  assert.match(out, /\[SWEET\]/);
...
test('statusline.sh renders savings by default when env var is unset', (tmp) => {
  ...
  delete env.SWEET_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'sweet-statusline.sh')], {
    encoding: 'utf8', env,
  });
  assert.match(out, /\[SWEET\]/);
...
test('statusline.sh omits savings when SWEET_STATUSLINE_SAVINGS=0', (tmp) => {
  if (process.platform === 'win32') return;
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.sweet-active'), 'full');
  ...
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'sweet-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, SWEET_STATUSLINE_SAVINGS: '0' },
  });
  assert.match(out, /\[SWEET\]/);
...
test('statusline.sh omits savings when suffix file is missing (fresh install)', (tmp) => {
  ...
  // /sweet-stats has run. Default-on must NOT fabricate a number.
  const env = { ...process.env, CLAUDE_CONFIG_DIR: claudeDir };
  delete env.SWEET_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'sweet-statusline.sh')], {
    encoding: 'utf8', env,
...
test('statusline.sh strips control bytes from suffix', (tmp) => {
  ...
  fs.writeFileSync(path.join(claudeDir, '.sweet-statusline-suffix'), '\x1b[31mEVIL');
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'sweet-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, SWEET_STATUSLINE_SAVINGS: '1' },
  });
...
test('appendFlag is symlink-safe (refuses symlinked target)', (tmp) => {
  ...
  const { appendFlag } = require(path.join(ROOT, 'src', 'hooks', 'sweet-config.js'));
...
test('mode tracker forwards --share to stats script', (tmp) => {
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, HOME: tmp },
    input: JSON.stringify({ prompt: '/sweet-stats --share', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
...
test('--all lifetime output labels the % as output reduction, not usage', (tmp) => {
  ...
  fs.writeFileSync(
    path.join(claudeDir, '.sweet-history.jsonl'),
...
// via the .sweet-mode-log.jsonl transition log — never to whatever mode the
...
test('attributes tokens to the mode active when each message happened (#601)', (tmp) => {
  ...
  // 300 verbose tokens BEFORE sweet was activated, 350 after.
  const sess = makeSession(tmp, [
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.sweet-mode-log.jsonl'),
  ...
  assert.match(out, /sweet off:\s+300 tokens \(no benchmark estimate\)/);
  ...
  const hist = fs.readFileSync(path.join(claudeDir, '.sweet-history.jsonl'), 'utf8')
    .split('\n').filter(Boolean).map(l => JSON.parse(l));
  assert.strictEqual(hist[hist.length - 1].est_saved_tokens, 650);
});

test('credits sweet spans even after mode is turned off mid-session (#601)', (tmp) => {
  const now = Date.now();
  const iso = (minAgo) => new Date(now - minAgo * 60_000).toISOString();
  const sess = makeSession(tmp, [
  ...
  // No .sweet-active flag — sweet is off at stats time. The old behavior
  // printed "Sweet not active this session." and logged zero savings.
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  assert.doesNotMatch(out, /Sweet not active this session/);
  assert.match(out, /full:\s+350 tokens \(est\. 650 saved\)/);
  assert.match(out, /Est\. tokens saved:\s+650\b/);
});
...
test('mode tracker logs timestamped transitions, deduping unchanged modes (#601)', (tmp) => {
  ...
  const logPath = path.join(claudeDir, '.sweet-mode-log.jsonl');
  const rows = () => fs.readFileSync(logPath, 'utf8').split('\n').filter(Boolean).map(l => JSON.parse(l));

  run('/sweet ultra');
  run('/sweet ultra'); // unchanged — must not append a duplicate row
  assert.strictEqual(rows().length, 1);
  assert.strictEqual(rows()[0].mode, 'ultra');
  assert.strictEqual(rows()[0].prev, 'full');
  assert.ok(Number.isFinite(rows()[0].ts));

  run('/sweet off'); // deactivation is a transition too
...
test('excludes tokens that predate a mid-session flag write with no log (#601)', (tmp) => {
  ...
  assert.doesNotMatch(out, /Est\. without sweet/);
});

// Honesty invariant: the shipped COMPRESSION table must stay empty until a
// Korean benchmark run is committed. If someone re-adds the inherited English
// 0.65 (or any unmeasured guess), this fails.
test('ships no built-in compression ratio — estimates require measured data', (tmp) => {
  const sess = makeSession(tmp, [
    { type: 'assistant', message: { usage: { output_tokens: 350 } } },
  ]);
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.sweet-active'), 'full');
  const env = { ...process.env, CLAUDE_CONFIG_DIR: claudeDir };
  delete env.SWEET_COMPRESSION_RATIOS;
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env,
  });
  assert.doesNotMatch(out, /Est\. without sweet/);
  assert.doesNotMatch(out, /Est\. tokens saved/);
  assert.match(out, /no mode has a committed benchmark yet/);
  // Real measurements still work.
  assert.match(out, /Output tokens:\s+350/);
});

// Malformed or out-of-range overrides must not produce an estimate.
test('rejects invalid SWEET_COMPRESSION_RATIOS values', (tmp) => {
  const sess = makeSession(tmp, [
    { type: 'assistant', message: { usage: { output_tokens: 350 } } },
  ]);
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.sweet-active'), 'full');
  for (const bad of ['not json', '[0.65]', '{"full":1}', '{"full":0}', '{"full":"0.65"}', '{"nope":0.5}']) {
    const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
      encoding: 'utf8',
      env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, SWEET_COMPRESSION_RATIOS: bad },
    });
    assert.doesNotMatch(out, /Est\. tokens saved/, `should reject ${bad}`);
  }
});
