...
const { safeWriteFlag, readFlag, VALID_MODES } = require('../src/hooks/sweet-config');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmpBase = fs.mkdtempSync(path.join(os.tmpdir(), 'sweet-symlink-test-'));
  try {
    fn(tmpBase);
    passed++;
    console.log(`  ✓ ${name}`);
...
test('writes flag in normal (non-symlinked) directory', (tmp) => {
  ...
  const flagPath = path.join(flagDir, '.sweet-active');
...
test('writes flag when parent directory is a symlink owned by current user', (tmp) => {
  ...
  const flagPath = path.join(symlinkDir, '.sweet-active');
  safeWriteFlag(flagPath, 'ultra');

  // Flag should exist in the real directory
  const realFlagPath = path.join(realDir, '.sweet-active');
...
test('readFlag works through symlinked parent directory', (tmp) => {
  ...
  const result = readFlag(path.join(symlinkDir, '.sweet-active'));
...
test('refuses flag file that is itself a symlink (even through symlinked parent)', (tmp) => {
  ...
  safeWriteFlag(path.join(symlinkDir, '.sweet-active'), 'full');
...
test('readFlag refuses flag file that is a symlink', (tmp) => {
  ...
  fs.symlinkSync(secretFile, path.join(realDir, '.sweet-active'));

  const result = readFlag(path.join(realDir, '.sweet-active'));
...
test('symlink to nonexistent target silently fails', (tmp) => {
  ...
  assert.strictEqual(fs.existsSync(path.join(symlinkDir, '.sweet-active')), false);
});
...
test('safeWriteFlag no longer has blanket symlink parent refusal', (tmp) => {
  ...
  const source = fs.readFileSync(
    path.join(__dirname, '..', 'src', 'hooks', 'sweet-config.js'), 'utf8'
  );
