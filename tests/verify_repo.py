#!/usr/bin/env python3
"""Local verification runner for sweet install surfaces."""
...
def verify_skill_frontmatter_upload_compatibility() -> None:
    ...
    skill_paths = [
        ROOT / "skills/sweet/SKILL.md",
        ROOT / "skills/sweet-commit/SKILL.md",
        ROOT / "skills/sweet-help/SKILL.md",
        ROOT / "skills/sweet-review/SKILL.md",
        ROOT / "skills/sweet-compress/SKILL.md",
    ]
...
def verify_synced_files() -> None:
    ...
    skill_source = ROOT / "skills/sweet/SKILL.md"

    skill_copies = [
        ROOT / "plugins/sweet-agent/skills/sweet/SKILL.md",
    ]
    ...
    with zipfile.ZipFile(ROOT / "dist" / "sweet.skill") as archive:
        ensure("sweet/SKILL.md" in archive.namelist(), "sweet.skill missing sweet/SKILL.md")
        ensure(
            archive.read("sweet/SKILL.md").decode("utf-8")
            == skill_source.read_text(encoding="utf-8"),
            "sweet.skill payload mismatch",
        )

    ensure(
        (ROOT / "bin" / "install.js").exists(),
        "bin/install.js missing — package.json bin entry would break npx sweet",
    )
    ensure(
    ...
    print("Synced copies, sweet.skill zip, and installer entrypoints OK")
...
def verify_manifests_and_syntax() -> None:
    ...
    manifest_paths = [
        ...
        ROOT / "plugins/sweet-agent/.codex-plugin/plugin.json",
    ]
    for path in manifest_paths:
        read_json(path)

    run(["node", "--check", "src/hooks/sweet-config.js"])
    run(["node", "--check", "src/hooks/sweet-activate.js"])
    run(["node", "--check", "src/hooks/sweet-mode-tracker.js"])
    run(["node", "--check", "src/hooks/sweetcrew-model-overrides.js"])
    ...
    run(["bash", "-n", "src/hooks/sweet-statusline.sh"])

    # Ensure install/uninstall scripts include sweet-config.js
    ...
    ensure("sweet-config.js" in install_sh, "install.sh missing sweet-config.js")
    ensure("sweet-config.js" in uninstall_sh, "uninstall.sh missing sweet-config.js")
...
def verify_powershell_static() -> None:
    ...
    statusline_text = (ROOT / "src/hooks/sweet-statusline.ps1").read_text(encoding="utf-8")

    ensure("sweet-config.js" in install_text, "install.ps1 missing sweet-config.js")
    ensure("sweet-config.js" in uninstall_text, "uninstall.ps1 missing sweet-config.js")
    ensure("sweet-statusline.ps1" in install_text, "install.ps1 missing statusline.ps1")
    ensure("sweet-statusline.ps1" in uninstall_text, "uninstall.ps1 missing statusline.ps1")
    ...
    ensure("[SWEET" in statusline_text, "sweet-statusline.ps1 missing badge output")

    print("Windows install path statically wired")


def load_compress_modules():
    sys.path.insert(0, str(ROOT / "skills/sweet-compress"))
...
def verify_compress_fixtures() -> None:
    ...
    fixtures = sorted((ROOT / "tests/sweet-compress").glob("*.original.md"))
    ensure(fixtures, "No sweet-compress fixtures found")
    ...
    print(f"Validated {len(fixtures)} sweet-compress fixture pairs")
...
def verify_compress_cli() -> None:
    ...
    skip_result = run(
        ...
        cwd=ROOT / "skills/sweet-compress",
        check=False,
    )
...
def verify_hook_install_flow() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="sweet-verify-") as temp_root:
        ...
        ensure("SWEET MODE ACTIVE" in activate.stdout, "activation output missing sweet banner")
        ensure("STATUSLINE SETUP NEEDED" not in activate.stdout, "activation should stay quiet when custom statusline exists")
        ensure((claude_dir / ".sweet-active").read_text(encoding="utf-8") == "full", "activation flag should default to full")

        # Test configurable default mode via SWEET_DEFAULT_MODE env var
        activate_custom = run(
            ["node", "src/hooks/sweet-activate.js"],
            env={**hook_env, "SWEET_DEFAULT_MODE": "ultra"},
        )
        ensure("SWEET MODE ACTIVE" in activate_custom.stdout, "activation with custom default missing banner")
        ensure(
            (claude_dir / ".sweet-active").read_text(encoding="utf-8") == "ultra",
            "SWEET_DEFAULT_MODE=ultra should set flag to ultra",
        )
        # Test "off" mode — activation skipped, flag removed
        activate_off = run(
            ["node", "src/hooks/sweet-activate.js"],
            env={**hook_env, "SWEET_DEFAULT_MODE": "off"},
        )
        ensure("SWEET MODE ACTIVE" not in activate_off.stdout, "off mode should not emit sweet banner")
        ensure(not (claude_dir / ".sweet-active").exists(), "off mode should remove flag file")

        # Test mode tracker with /sweet when default is off — should NOT write flag
        subprocess.run(
            ["node", "src/hooks/sweet-mode-tracker.js"],
            cwd=ROOT,
            env={**os.environ, **hook_env, "SWEET_DEFAULT_MODE": "off"},
            text=True,
            encoding="utf-8",
            input='{"prompt":"/sweet"}',
            capture_output=True,
        ...
        ensure(not (claude_dir / ".sweet-active").exists(), "/sweet with off default should not write flag")

        # Reset back to full for subsequent tests
        (claude_dir / ".sweet-active").write_text("full")
        ...
        ultra_prompt = subprocess.run(
            ...
            text=True,
            encoding="utf-8",
            input='{"prompt":"/sweet ultra"}',
            capture_output=True,
            check=True,
        )
        ensure(
            "SWEET MODE ACTIVE (ultra)" in ultra_prompt.stdout,
            "mode tracker should emit active-mode reinforcement",
        )
        ensure((claude_dir / ".sweet-active").read_text(encoding="utf-8") == "ultra", "mode tracker did not record ultra")
        ...
        ensure(not (claude_dir / ".sweet-active").exists(), "normal mode should remove flag file")

        (claude_dir / ".sweet-active").write_text("ultra")
        statusline = run(
            ["bash", "src/hooks/sweet-statusline.sh"],
            env=hook_env,
        )
        ensure("[SWEET:ULTRA]" in statusline.stdout, "statusline badge output mismatch")
        ...
        ensure(settings_after == existing_settings, "uninstall.sh did not restore non-sweet settings")
        ensure(not (claude_dir / ".sweet-active").exists(), "uninstall.sh should remove flag file")

    with tempfile.TemporaryDirectory(prefix="sweet-verify-fresh-") as temp_root:
        ...
        activate = run(["node", "src/hooks/sweet-activate.js"], env=hook_env)
