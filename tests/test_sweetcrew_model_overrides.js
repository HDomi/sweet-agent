#!/usr/bin/env node
// Tests for src/hooks/sweetcrew-model-overrides.js
// Run: node tests/test_sweetcrew_model_overrides.js
...
const { patchFrontmatterModel, resolvePluginRoot, applyOverrides, AGENT_ENV_MAP } =
  require('../src/hooks/sweetcrew-model-overrides');
...
test('preserves all other frontmatter lines', () => {
  ...
  assert.ok(out.includes('name: sweetcrew-reviewer'), 'name line lost');
...
function withTmpPlugin(fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sweet-override-test-'));
...
test('replaces reviewer model when SWEETCREW_REVIEWER_MODEL set', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { SWEETCREW_REVIEWER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), 'utf8');
    assert.ok(out.includes('model: sonnet'), 'reviewer model not patched');
  });
});

test('replaces investigator model when SWEETCREW_INVESTIGATOR_MODEL set', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'sweetcrew-investigator.md'), INVESTIGATOR_FM, 'utf8');
    applyOverrides(root, { SWEETCREW_INVESTIGATOR_MODEL: 'opus' });
    const out = fs.readFileSync(path.join(agentsDir, 'sweetcrew-investigator.md'), 'utf8');
    assert.ok(out.includes('model: opus'), 'investigator model not patched');
  });
});

test('inserts builder model when SWEETCREW_BUILDER_MODEL set and no model line', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'sweetcrew-builder.md'), BUILDER_FM, 'utf8');
    applyOverrides(root, { SWEETCREW_BUILDER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'sweetcrew-builder.md'), 'utf8');
...
test('blank env var is no-op', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { SWEETCREW_REVIEWER_MODEL: '' });
    const out = fs.readFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), 'utf8');
...
test('env var with newline in value is ignored', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { SWEETCREW_REVIEWER_MODEL: 'so\nnnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), 'utf8');
...
test('env var with control character in value is ignored', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { SWEETCREW_REVIEWER_MODEL: 'son\x00net' });
    const out = fs.readFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), 'utf8');
...
test('missing agents dir (non-plugin layout) is silent no-op', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sweet-nolayout-'));
  try {
    assert.doesNotThrow(() => {
      applyOverrides(tmp, { SWEETCREW_REVIEWER_MODEL: 'sonnet' });
    });
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
  }
...
test('body content preserved after model patch', () => {
  withTmpPlugin((root, agentsDir) => {
    ...
    fs.writeFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), content, 'utf8');
    applyOverrides(root, { SWEETCREW_REVIEWER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'sweetcrew-reviewer.md'), 'utf8');
