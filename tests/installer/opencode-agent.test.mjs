...
// Claude Code accepts. Copying agents/sweetcrew-*.md verbatim into
// ~/.config/opencode/agents/ broke opencode startup with:
//   Configuration is invalid at .../sweetcrew-reviewer.md
...
const SHIPPED_AGENT_FILES = ['sweetcrew-investigator.md', 'sweetcrew-builder.md', 'sweetcrew-reviewer.md'];
...
// This is the RED-state proof: each `agents/sweetcrew-*.md` in the repo today
...
test('all shipped sweetcrew agent files contain offending tools array (RED proof)', () => {
...
test('all shipped sweetcrew agent files become opencode-safe after transform (GREEN proof)', () => {
  for (const f of SHIPPED_AGENT_FILES) {
    const src = fs.readFileSync(path.join(REPO_ROOT, 'agents', f), 'utf8');
    const out = stripOpencodeAgentTools(src);
    const fm = frontmatter(out);
    ...
    assert.match(fm, /^name: sweetcrew-/m, `${f}: name field preserved`);
