# sweet — installer shim (Windows / PowerShell).
#
...
function Install-Sweet {
  param(
    [string[]]$InstallerArgs = @()
  )

  $ErrorActionPreference = "Stop"
  $Repo = "HDomi/sweet-agent"

  # Require Node ≥18.
  $node = Get-Command node -ErrorAction SilentlyContinue
  if (-not $node) {
    Write-Error @"
sweet: Node.js (>=18) required. Install:
...
"@
  ...
  if ($nodeMajor -lt 18) {
    Write-Error "sweet: Node $nodeMajor too old. Need Node >=18. Upgrade: https://nodejs.org"
    exit 1
  }
  ...
  if (-not $npx) {
    Write-Error "sweet: npx required (ships with Node >=18). Reinstall Node.js."
    exit 1
  }
...
Install-Sweet -InstallerArgs $args
