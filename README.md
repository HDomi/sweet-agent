...
> [!IMPORTANT]
> **코드 품질은 안 건드린다.** 짧아지는 건 에이전트가 오빠한테 *말하는 부분*뿐이다.
>
> - **파일에 들어가는 건 그대로.** 코드는 물론이고 주석, docstring, 로그 문자열, 테스트 이름, 문서까지 — 레포에 원래 쓰던 언어를 따라간다. 영어 주석 프로젝트에 한국어 주석이 섞이지 않는다.
> - **생략은 말투에만.** 실패한 테스트, 에러, 건너뛴 작업, 남은 위험은 짧게 만들려고 빼지 않는다. 안 해본 걸 해봤다고 쓰지도 않는다.
> - **위험한 부분은 압축을 끈다.** 보안 경고, 되돌릴 수 없는 작업(`DROP TABLE`, force push, 배포), 순서가 중요한 절차는 자동으로 또박또박한 문장으로 바뀐다.
> - **모델도 그대로.** 추론 예산이나 모델을 바꾸지 않는다. 설치해도 에이전트가 "생각을 덜 하게" 되는 설정은 하나도 없다.
>
> 코드 작업 요청("이 함수 간단히 해줘", "출력 토큰 줄이는 코드 써줘")을 모드 전환으로 착각하지도 않는다.
...
| `sweet-shrink` | MCP 미들웨어(기본 꺼짐). 아무 MCP 서버나 감싸서 tool description 의 군더더기만 지운다. 뜻이 걸린 단어(`might`, `only`, `literally`, `make sure`...)와 파라미터 설명은 손대지 않는다. `src/mcp-servers/sweet-shrink/` 에 소스가 있고, npm 에는 아직 배포 안 됨. |
