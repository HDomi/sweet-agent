...
#   curl -fsSL https://raw.githubusercontent.com/HDomi/sweet-agent/main/install.sh | bash -s -- --all
#
# Local clone:
...
if ! command -v node >/dev/null 2>&1; then
  echo "sweet: Node.js (≥18) required. Install:" >&2
...
if [ "$NODE_MAJOR" -lt 18 ]; then
  echo "sweet: Node $NODE_MAJOR too old. Need Node ≥18." >&2
...
if ! command -v npx >/dev/null 2>&1; then
  echo "sweet: npx required (ships with Node ≥18). Reinstall Node.js." >&2
  exit 1
fi
