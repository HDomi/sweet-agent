#!/usr/bin/env node
// sweet-shrink — MCP middleware that proxies an upstream MCP server and
// compresses prose fields so the model sees fewer tokens.
//
// Usage:
//   sweet-shrink <upstream-command> [...args]
//
// Example wrapping the filesystem MCP server:
//   "mcpServers": {
//     "fs-shrunk": {
//       "command": "npx",
//       "args": ["sweet-shrink", "npx", "@modelcontextprotocol/server-filesystem", "/some/path"]
//     }
//   }
//
// Compression is applied to:
//   - "description" fields in tools/list, prompts/list, resources/list responses
//   - same boundaries as sweet-compress: code, URLs, paths, identifiers preserved
//
...
//   SWEET_SHRINK_FIELDS   comma-separated extra field names to compress
//                           (default: description)
//   SWEET_SHRINK_DEBUG=1  log compression deltas to stderr
...
if (args.length === 0) {
  process.stderr.write('sweet-shrink: missing upstream command.\n');
  process.stderr.write('Usage: sweet-shrink <upstream-command> [...args]\n');
  process.exit(2);
}

const debug = process.env.SWEET_SHRINK_DEBUG === '1';
const fields = (process.env.SWEET_SHRINK_FIELDS || 'description')
...
upstream.on('error', err => {
  process.stderr.write(`sweet-shrink: failed to spawn upstream: ${err.message}\n`);
  process.exit(1);
});
            ...
            if (out !== before) {
              ...
              if (debug) {
                process.stderr.write(
                  `[sweet-shrink] ${arrayName}.${item.name || '?'}.${field}: ` +
