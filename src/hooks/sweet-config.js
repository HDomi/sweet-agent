#!/usr/bin/env node
// sweet — shared configuration resolver
//
// Resolution order for default mode:
//   1. SWEET_DEFAULT_MODE environment variable
...
//      - $XDG_CONFIG_HOME/sweet/config.json (any platform, if set)
//      - ~/.config/sweet/config.json (macOS / Linux fallback)
//      - %APPDATA%\sweet\config.json (Windows fallback)
//   4. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'sweet');
  }
  ...
  return path.join(os.homedir(), '.config', 'sweet');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.SWEET_DEFAULT_MODE;
...
// flag path (~/.claude/.sweet-active) with a symlink to clobber other files.
//
...
//
// Set SWEET_DEBUG=1 to emit stderr diagnostics when flag writes are refused.
//
// Silent-fails on any filesystem error — the flag is best-effort.
function safeWriteFlag(flagPath, content) {
  const debug = process.env.SWEET_DEBUG === '1';
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (!realStat.isDirectory()) {
          if (debug) process.stderr.write(`[sweet] safeWriteFlag: symlink target ${realFlagDir} is not a directory\n`);
          return;
        }
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[sweet] safeWriteFlag: symlink target ${realFlagDir} owned by uid ${realStat.uid}, not current user ${process.getuid()}\n`);
            return;
          }
        } else {
          ...
          if (!normalizedReal.toLowerCase().startsWith(normalizedHome.toLowerCase() + path.sep) &&
            ...
            if (debug) process.stderr.write(`[sweet] safeWriteFlag: symlink target ${normalizedReal} is outside home directory ${normalizedHome}\n`);
            return;
          }
        }
      } else {
    ...
    const tempPath = path.join(realFlagDir, `.sweet-active.${process.pid}.${Date.now()}`);
...
// ($CLAUDE_CONFIG_DIR/.sweet-history.jsonl).
//
  ...
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[sweet] appendFlag: symlink target ${realDir} owned by uid ${realStat.uid}\n`);
            return;
          }
        } else {
          const home = os.homedir();
...
// append {ts, mode, prev} to $CLAUDE_CONFIG_DIR/.sweet-mode-log.jsonl so
// sweet-stats can attribute output tokens to the mode that was active when
// each message was generated, instead of whatever mode the flag holds at
// stats time. mode/prev are a VALID_MODES string or null (null = sweet off).
...
const MODE_LOG_BASENAME = '.sweet-mode-log.jsonl';

function recordModeChange(claudeDir, newMode) {
  try {
    const current = readFlag(path.join(claudeDir, '.sweet-active'));
