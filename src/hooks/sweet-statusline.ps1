...
$Flag = Join-Path $ClaudeDir ".sweet-active"
...
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "full") {
    [Console]::Write("${Esc}[38;5;172m[SWEET]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;172m[SWEET:$Suffix]${Esc}[0m")
}

# Savings suffix: on by default. Opt out via SWEET_STATUSLINE_SAVINGS=0.
# Reads a pre-rendered string written by sweet-stats.js. Refuses reparse
# points and strips control bytes (matches statusline.sh hardening). Until
# /sweet-stats has run at least once, the suffix file is absent and nothing
# is rendered — safe default for fresh installs.
if ($env:SWEET_STATUSLINE_SAVINGS -ne "0") {
    $SavingsFile = Join-Path $ClaudeDir ".sweet-statusline-suffix"
