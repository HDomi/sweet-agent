#!/usr/bin/env node
// sweet-stats — read the active Claude Code session log, print real token
// usage plus an estimated savings figure from the benchmark in benchmarks/.
//
// Run directly:    node hooks/sweet-stats.js
// Inside Claude:   /sweet-stats triggers this via the UserPromptSubmit hook.
...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag, VALID_MODES, MODE_LOG_BASENAME } = require('./sweet-config');

// Mean per-task output savings, keyed by mode, from a committed benchmark run
// in benchmarks/results/*.json.
//
// EMPTY ON PURPOSE. The 0.65 figure this project inherited was measured on
// English caveman prose; it does not carry over to Korean 반말, and reprinting
// it here would be a fabricated number. Run benchmarks/run.py with the Korean
// prompt set, commit the result, then add the measured ratio below.
//
// SWEET_COMPRESSION_RATIOS supplies ratios without editing this file — a JSON
// object like {"full":0.65}. Entries outside the exclusive range (0,1) or for
// unknown modes are ignored.
function parseCompressionRatios(raw) {
  const out = {};
  if (!raw) return out;
  try {
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return out;
    for (const [mode, ratio] of Object.entries(parsed)) {
    ...
    // Malformed JSON → no ratios, same as unset
  }
  return out;
}

const COMPRESSION = parseCompressionRatios(process.env.SWEET_COMPRESSION_RATIOS);
...
// Detect *.original.md / *.md pairs left behind by sweet-compress. The
...
// estimate (verbose tokens counted as compressed) or zero it (sweet tokens
// counted as uncompressed). The mode tracker + SessionStart hook append
// {ts, mode, prev} rows to .sweet-mode-log.jsonl on every actual transition;
...
// a mode string or 'none' (sweet inactive).
...
// would-be OUTPUT tokens that sweet avoided. That is the only ratio we can
...
// Pro/Max limits, and are not reduced by sweet, so real limit relief is far
...
function formatHistory({ sessions, outputTokens, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nSweet Stats — Lifetime${window}\n${sep}\nNo sessions logged yet — run /sweet-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ outputTokens, turns, mode, model, attribution }) {
  if (turns === 0) {
    return '🪨 sweet armed but no turns yet — sweet.sh';
  }
  ...
  if (estSavedTokens > 0) {
    ...
    return `🪨 Saved ${estSavedTokens.toLocaleString()} output tokens${usd} across ${turns} turns this session — sweet.sh`;
  }
  return `🪨 ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session — sweet.sh`;
}
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed, attribution }) {
  ...
  if (!uniform) {
    ...
    for (const key of activeKeys) {
      ...
      const label = key === 'none' ? 'sweet off' : key;
      const note = r != null
    ...
    // sessions and count against Pro/Max limits) are untouched by sweet —
    // any session-usage % would overstate real limit relief. See
    // docs/HONEST-NUMBERS.md.
    footer += ' Reduction is of output tokens only; input/cache usage is unchanged.';
    savings = (`Est. without sweet:  ${estNormal.toLocaleString()}\n` +
              `Est. tokens saved:     ${estSaved.toLocaleString()} (~${Math.round(ratio * 100)}% of output)\n` +
              usdLine).replace(/\n$/, '');
  } else if (mode && mode !== 'off') {
    const measured = Object.keys(COMPRESSION);
    savings = measured.length > 0
      ? `No savings estimate for '${mode}' mode — measured data exists only for: ${measured.join(', ')}.`
      : `No savings estimate — no mode has a committed benchmark yet. Run benchmarks/run.py, then set SWEET_COMPRESSION_RATIOS or add the ratio to sweet-stats.js.`;
  } else {
    savings = 'Sweet not active this session.';
  }
...
function main() {
  ...
  const historyPath = path.join(claudeDir, '.sweet-history.jsonl');
  ...
  if (all || sinceArg) {
    ...
    if (sinceArg && sinceMs === null) {
      process.stderr.write(`sweet-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`);
      process.exit(2);
    }
  ...
  if (!sessionFile) {
    process.stderr.write('sweet-stats: no Claude Code session found.\n');
    process.exit(1);
  }
  ...
  // /sweet-stats calls in one session emit multiple lines for the same
  ...
  if (parsed.turns > 0) {
    ...
    // cat without parsing JSONL. Updated on every /sweet-stats run.
    // Routed through safeWriteFlag — the suffix path is predictable and
    // user-owned, same symlink-clobber surface as the .sweet-active flag.
    const agg = aggregateHistory(historyPath, null);
    const suffix = agg.estSavedTokens > 0 ? `⛏  ${humanizeTokens(agg.estSavedTokens)}` : '';
    safeWriteFlag(path.join(claudeDir, '.sweet-statusline-suffix'), suffix);
  }
