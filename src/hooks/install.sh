#!/bin/bash
# sweet — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash src/hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/HDomi/sweet-agent/main/src/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the sweet hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/HDomi/sweet-agent/main/hooks"

HOOK_FILES=("package.json" "sweet-config.js" "sweet-activate.js" "sweet-mode-tracker.js" "sweet-stats.js" "sweet-statusline.sh" "sweetcrew-model-overrides.js")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if SWEET_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.SWEET_SETTINGS, 'utf8'));
      const hasSweetHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('sweet'))
        );
      process.exit(
        hasSweetHook('SessionStart') &&
        hasSweetHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Sweet hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/sweet-activate.js" ]; then
  echo "Reinstalling sweet hooks (--force)..."
else
  echo "Installing sweet hooks..."
fi
...
chmod +x "$HOOKS_DIR/sweet-statusline.sh"
...
SWEET_SETTINGS="$SETTINGS" SWEET_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.SWEET_SETTINGS;
  const hooksDir = process.env.SWEET_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/sweet-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load sweet rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/sweet-activate.js\"',
        timeout: 5,
  ...
  // UserPromptSubmit — track mode changes when user types /sweet commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/sweet-mode-tracker.js\"',
        timeout: 5,
  ...
  // Statusline — wire sweet badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — sweet badge NOT added.');
...
echo "  - SessionStart hook: auto-loads sweet rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/sweet lite, /sweet ultra, /sweet-commit, etc.)"
echo "  - Statusline badge: shows [SWEET] or [SWEET:ULTRA] etc."
