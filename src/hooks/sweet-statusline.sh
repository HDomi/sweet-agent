#!/bin/bash
# sweet — statusline badge script for Claude Code
# Reads the sweet mode flag file and outputs a colored badge.
#
# Usage in ~/.claude/settings.json:
#   "statusLine": { "type": "command", "command": "bash /path/to/sweet-statusline.sh" }
#
# Plugin users: Claude will offer to set this up on first session.
# Standalone users: install.sh wires this automatically.

FLAG="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.sweet-active"
...
if [ -z "$MODE" ] || [ "$MODE" = "full" ]; then
  printf '\033[38;5;172m[SWEET]\033[0m'
else
  SUFFIX=$(printf '%s' "$MODE" | tr '[:lower:]' '[:upper:]')
  printf '\033[38;5;172m[SWEET:%s]\033[0m' "$SUFFIX"
fi

# Savings suffix: on by default. Opt out via SWEET_STATUSLINE_SAVINGS=0.
# Reads a pre-rendered string written by sweet-stats.js so we don't shell out
...
# ANSI escape codes otherwise). Until /sweet-stats has run at least once,
# the suffix file is absent and nothing is rendered — so the default is safe
# for fresh installs (no fake number, no crash).
if [ "${SWEET_STATUSLINE_SAVINGS:-1}" != "0" ]; then
  SAVINGS_FILE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.sweet-statusline-suffix"
