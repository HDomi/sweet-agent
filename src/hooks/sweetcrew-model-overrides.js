#!/usr/bin/env node
// sweetcrew model overrides — patch installed agent frontmatter from env vars.
//
// Called by sweet-activate.js early in SessionStart so users can pin
// per-agent models without shadow-copying entire agent files.
//
// Env vars:
//   SWEETCREW_REVIEWER_MODEL    → agents/sweetcrew-reviewer.md
//   SWEETCREW_BUILDER_MODEL     → agents/sweetcrew-builder.md
//   SWEETCREW_INVESTIGATOR_MODEL → agents/sweetcrew-investigator.md
//
// Rules:
//   - Unset / blank → no-op.
...
const AGENT_ENV_MAP = [
  { envVar: 'SWEETCREW_REVIEWER_MODEL',     file: path.join('agents', 'sweetcrew-reviewer.md') },
  { envVar: 'SWEETCREW_BUILDER_MODEL',      file: path.join('agents', 'sweetcrew-builder.md') },
  { envVar: 'SWEETCREW_INVESTIGATOR_MODEL', file: path.join('agents', 'sweetcrew-investigator.md') },
];
