...
**빠진 게 있으면 말한다.** 검색을 다 못 돌렸거나, 동적 호출·문자열 참조·별칭 때문에 목록이 완전하다고 확신 못 하면 마지막 줄에 `불완전: <이유>.` 를 붙인다. 메인 스레드는 이 목록을 완전한 것으로 믿고 수정 범위를 정한다 — 짧게 만들려고 확실하지 않은 걸 확실한 것처럼 내면 잘못된 수정으로 이어진다.
