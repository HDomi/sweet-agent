#!/usr/bin/env python3
"""
Sweet Memory Compression Orchestrator
...
def backup_dir_for(filepath: Path) -> Path:
    ...
    `.original.md` copies as live files. Base dir is platform-aware:
      - Windows: %LOCALAPPDATA%\\sweet-compress\\backups
      - else:    $XDG_DATA_HOME/sweet-compress/backups if set,
                 else ~/.local/share/sweet-compress/backups
    ...
    if os.name == "nt" or sys.platform == "win32":
        ...
        base = base / "sweet-compress" / "backups"
    else:
    ...
    if api_key:
        try:
            ...
            msg = client.messages.create(
                model=os.environ.get("SWEET_MODEL", "claude-sonnet-4-5"),
                max_tokens=8192,
...
def build_compress_prompt(original: str) -> str:
    return f"""
Compress this markdown into sweet format.
...
def build_fix_prompt(original: str, compressed: str, errors: List[str]) -> str:
    ...
    return f"""You are fixing a sweet-compressed markdown file. Specific validation errors were found.

CRITICAL RULES:
- DO NOT recompress or rephrase the file
- ONLY fix the listed errors — leave everything else exactly as-is
- The ORIGINAL is provided as reference only (to restore missing content)
- Preserve sweet style in all untouched sections
...
def compress_file(filepath: Path) -> bool:
    ...
    if compressed_body.strip() == body.strip():
        ...
        print("   already in sweet form. Original file is untouched (no backup created).")
        return False
